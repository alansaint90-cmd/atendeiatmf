...
test("API protege leitura e gravação antes de acessar banco", async () => {
  ...
  try {
    ...
    assert.equal((await agentStatus(new Request("http://localhost/api/settings/agent"))).status, 401);
    ...
    assert.equal((await PUT(new Request("http://localhost/api/settings/integrations", { method: "PUT", headers, body: "x".repeat(1048577) }))).status, 413);
  } finally {
